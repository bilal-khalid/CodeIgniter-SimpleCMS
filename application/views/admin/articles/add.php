...
<div class="row">
    <div class="col-12">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
            <h3 class="my-2">Add Article</h3>
        </div>
        <?= validation_errors('<div class="alert alert-danger" role="alert">', '</div>'); ?>
        <form action="<?= site_url('admin/articles/add') ?>" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="body" id="body" rows="10" required><?= set_value('body') ?></textarea>
            </div>
            <div class="form-group">
                ...
                <select class="form-control" name="user_id" id="user_id" required>
                    ...
                    foreach ($users as $user) {
                        echo '<option value="' . $user->id . '" ' .
                            (set_value('user_id') == $user->id ? 'selected' : '') .
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="access" id="access" required>
                    <option value=""></option>
                    <option value="0" <?= set_value('access') == '0' ? 'selected' : '' ?>>Everyone</option>
                </select>
                ...
                <div class="form-check-inline">
                    <label class="form-check-label">
                        <input type="radio" class="form-check-input" name="is_published" value="1" 
                            <?= set_value('is_published') === '' || set_value('is_published') === '1' ? 'checked' : '' ?>> Yes
                    </label>
                </div>
                <div class="form-check-inline">
                    <label class="form-check-label">
                        <input type="radio" class="form-check-input" name="is_published" value="0"
                            <?= set_value('is_published') === '0' ? 'checked' : '' ?>> No
                    </label>
                </div>
            </div>
            <div>
